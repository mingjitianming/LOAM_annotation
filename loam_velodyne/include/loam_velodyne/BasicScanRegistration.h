...
namespace loam
{
  ...
  private:
    ...
    std::vector<float> _regionCurvature;      ///< point curvature buffer  //曲率半径从小到大排序
    ...
    std::vector<int> _scanNeighborPicked;     ///< flag if neighboring point was already picked //不能设置为特征点的点
  };
